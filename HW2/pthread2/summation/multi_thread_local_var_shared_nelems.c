...
void multi_thread_local_var(size_t iterations, size_t number, size_t nthreads, size_t cache_line_size, char output_opt)
{
    ...
    for (int it = 0; it < iterations; it++) {
        ...
        for(size_t t = 0; t < nthreads; t++)
        {
            ...
            pthread_create(&tid[t*cache_line_size], NULL, thread_loop_local, &slice_count[t*cache_line_size]);
        }
...
void *thread_loop_local(void *vargp)
{
    int myid = *((int *)vargp);
    ...
    // Critical Section
    printf("# Locked - %d #",myid);
    pthread_mutex_lock(&global_sum_lock);
    global_sum += local_sum;
    printf("# Unlocked - %d #",myid);
